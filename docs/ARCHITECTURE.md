...
| `pilot/`          | **Not part of the app.** A separate program that drives it over the API: an automated search that captures candidates, embeds and scores them, and steers the next generation. Runs in its own process and imports nothing from any module above — the HTTP transport is the whole of the contact, which is what keeps torch out of the GL process. See [SEARCH.md](SEARCH.md). |
